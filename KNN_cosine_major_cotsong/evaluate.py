...
import numpy as np
import os
from modelpre.model import knn_core, cosine_distance_core
...
def knn_predict_batch(X_train, y_train, X_test, k=3):
    """Dự đoán toàn bộ tập test bằng KNN Cosine Distance."""
    return np.array([knn_core(X_train, y_train, row, k=k) for row in X_test])
...
def auc_binary(y_true, y_scores):
    ...
    y_scores: điểm tin cậy (ở đây dùng tỉ lệ phiếu bầu từ K láng giềng Cosine).
    """
...
def cosine_vote_scores(X_train, y_train, X_test, k, target_class):
    """
    Tính score cho AUC bằng tỉ lệ phiếu bầu K láng giềng gần nhất (Cosine).
    Score = số láng giềng thuộc target_class / K
    """
    scores = []
    for row in X_test:
        distances  = cosine_distance_core(X_train, row)   # Dc nhỏ = gần nhất
        k_indices  = np.argsort(distances)[:k]
        k_labels   = y_train[k_indices]
        vote_ratio = np.mean(k_labels == target_class)
        scores.append(vote_ratio)
...
def run_k_fold_cv(X, y, classes, k_folds=5, k_knn=3, agg_method=None):
    """
    Chạy K-Fold CV cho KNN Cosine Distance.
    ...
    print(f"\n{'='*65}")
    print(f"  K-FOLD CROSS VALIDATION — KNN COSINE (K={k_knn}, Folds={k_folds})")
    print(f"{'='*65}")
    ...
    for i in range(k_folds):
        ...
        y_pred = knn_predict_batch(X_tr, y_tr, X_te, k=k_knn)
        ...
        # AUC — dùng tỉ lệ phiếu bầu Cosine làm score
        if not is_multiclass:
            scores    = cosine_vote_scores(X_tr, y_tr, X_te, k_knn, classes[1])
            ...
            for cls in classes:
                ...
                scores = cosine_vote_scores(X_tr, y_tr, X_te, k_knn, cls)
