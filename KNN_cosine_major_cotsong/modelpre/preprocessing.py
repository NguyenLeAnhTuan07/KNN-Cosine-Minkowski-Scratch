...
def run_preprocessing(csv_path, feature_path, training=True):
    ...
    # 1. Đọc danh sách tên đặc trưng từ file txt
    ...
    if training:
        X = df[all_names[:-1]].values.astype(float)  # Các cột đầu
        y = df[all_names[-1]].values                  # Cột cuối là nhãn

        # L2 Normalization: đưa mỗi vector về độ dài = 1
        # Phù hợp với Cosine vì Cosine chỉ quan tâm đến hướng vector
